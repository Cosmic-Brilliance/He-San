...
def get_checks():
    """Return a list of check names and functions."""
    return [
...
def assert_keys(obj: dict[str, Any], keys, name):
    """Assert that a dictionary contains the expected keys."""
...
def assert_non_empty_list(value: Any, name: str):
    """Assert that a value is a non-empty list."""
...
def to_date(value: Any, name: str) -> date:
    """Convert a value to a date object or raise AssertionError."""
    if isinstance(value, date):
        return value
    assert_type(value, str, name)
    try:
...
def validate_control_library():
    """Validate the control library artifact."""
    ...
    for idx, control in enumerate(data["controls"]):
        ...
        assert_non_empty_list(
            control["mapped_regimes"], f"control[{idx}].mapped_regimes"
        )
...
def validate_model_registry():
    """Validate the model registry artifact."""
    ...
    for m in data["models"]:
        ...
        assert_keys(
            m,
            [
                "model_id", "use_case", "risk_tier",
                "deployment_status", "controls", "validation"
            ],
            f"model:{m_id}"
        )
        ...
        next_due = to_date(
            m["validation"]["next_due"], f"model:{m_id}.validation.next_due"
        )
        if next_due < last_val:
            raise AssertionError(
                f"model:{m_id}.validation.next_due precedes last_validation"
            )
        assert_type(
...
def validate_control_references():
    """Validate that models reference existing controls."""
    ...
    for model in model_registry.get("models", []):
        ...
        for control_id in model.get("controls", []):
            ...
            if not control_id.strip():
                raise AssertionError(
                    f"model:{model_id} contains blank control id reference"
                )
            if control_id not in control_ids:
                raise AssertionError(
                    f"model:{model_id} references missing control id {control_id}"
                )


def validate_incident_taxonomy():
    """Validate the incident taxonomy artifact."""
...
def validate_annex_iv_template():
    """Validate the Annex IV dossier template."""
...
def validate_kpi_kri_schema():
    ...
    assert_keys(
        data, ["$schema", "title", "type", "properties", "required"], "kpi_kri_schema"
    )
    ...
    for section in ("kpis", "kris"):
        ...
        assert_keys(
            section_obj, ["type", "properties", "required"],
            f"kpi_kri_schema.properties.{section}"
        )
        assert_type(
            section_obj["properties"], dict,
...
def validate_rego_policy():
    """Validate Rego policy file contents."""
...
def validate_cae_specification():
    ...
    assert_keys(
        data, ["specification_version", "last_updated", "cae_definition"],
        "cae_specification"
    )
    assert_keys(
        data["cae_definition"], ["name", "description", "fields"], "cae_definition"
    )
...
def main(argv: list[str] | None = None):
    ...
    parser.add_argument(
        "--version", action="store_true", help="Print validator version and exit."
    )
    ...
    if args.list_checks:
        ...
        if args.json:
            print(
                json.dumps({"version": VALIDATOR_VERSION, "checks": checks}, indent=2)
            )
        else:
            for check in checks:
                print(check)
